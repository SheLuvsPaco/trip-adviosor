...
import { places, preferredCommonsFiles, ROUTE_SLUG } from "./config.mjs";
...
async function main() {
  ...
  for (const [position, place] of selectedPlaces.entries()) {
    ...
    if (retainedExisting.length >= 3 && existing?.selected_coordinate) {
    ...
    const selectedImages = retainedExisting;
    const unusedCandidates = imageCandidates
      ...
      .sort((a, b) => relevanceScore(place, b) - relevanceScore(place, a));
