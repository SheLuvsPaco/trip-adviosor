...
import { contextualImagePlaces, manualCoordinates } from "../route-05/config.mjs";
import {
  ARCHIVE_IDS,
  BOOKING_PRIORITIES,
  CORE_CONFIG,
...
const CAP_EXCEPTIONS = {
  9: {
    minutes: 330,
    reason: "Leesburg to Catoctin to Harrisburg to Jim Thorpe to Stroudsburg is the brief's iron-labour and foliage-train day. The measured total is 317 minutes, above the 270-minute standard cap, but no single leg exceeds 120 minutes because every drive is capped by a real stop, and sleeping in Stroudsburg rather than Jim Thorpe is what keeps Day 10 inside the rule."
  }
};
...
function scheduleItem([start, end, placeId, priority], placeById) {
  ...
  if (!place) throw new Error(`Missing Route 05 V2 place: ${placeId}`);
...
export async function buildEnergyRoute05({ root, routeDir }) {
  ...
  for (const id of CORE_IDS) {
    if (!placeById.has(id)) throw new Error(`Route 05 V2 core place is missing: ${id}`);
    if (!placeById.get(id).coordinates) throw new Error(`Route 05 V2 core place has no coordinates: ${id}`);
  }

  const newIds = new Set(NEW_CORE_IDS);
  ...
  const newImages = [...newIds].flatMap((placeId) => {
    ...
    if (selected.length < 3) throw new Error(`${placeId} has ${selected.length}/3 Route 05 V2 images.`);
  ...
  const route = {
    ...
    route: {
      id: ROUTE_ID,
      slug: "coal-veins-caverns-blue-ridge-secrets-loop",
      name: "Under the Mountain, Inside the Machine",
      short_name: "Under the Mountain",
      ...
      endpoint_city: "Charlottesville, VA",
      direction: "Boston to Danbury and the anthracite country around Scranton, south through Lancaster and Harrisburg to Harpers Ferry and the Shenandoah, on to Charlottesville and the Blue Ridge Tunnel, then north through Leesburg, Catoctin, Jim Thorpe, the Hudson Valley and the Connecticut iron hills to Boston",
      countries: ["US"],
      ...
      route_dna: "UNDERGROUND × INDUSTRY × RIDGE × RUIN × MACHINE",
      summary: "Under the mountain and inside the machine: a glacial rock chamber, a folk-religious hillside, a 300-foot descent into an anthracite mine, blast-furnace ruins, a preserved coal-patch town, a sixteen-acre boulder field, live steam through Lancaster County, an eighteenth-century religious settlement, Harpers Ferry's confluence and factory ruins, Luray's caverns, Skyline Drive, a Shenandoah trail ride, a quartzite scramble, a 4,273-foot unlit rail tunnel, Monticello's truth tour, an elite escape room, Orthodox Sunday, Catoctin's iron labour, an open-air foliage train, a rail bridge over the Hudson, a Connecticut iron mine and Jurassic footprints on the way home."
    },
    constraints: {
      travelers: 4,
      cars: 2,
      ...
      route_specific_drive_rule: "No planned uninterrupted run exceeds roughly 2.5 hours. Day 9 carries an explicit authorised total-driving exception because every one of its long legs is capped by a real stop.",
    ...
    experience_model: {
      ...
      museum_like_note: "Zero traditional passive museums or galleries in the active core. Culture is kept, but it now happens in mines, ruins, tunnels, sacred sites, working transport and real terrain.",
      bucket_note: "Bucket names follow the brief's own balance table: underground-industrial-infrastructure, nature-geology-scenic, high-impact-history-culture, sacred-spiritual, active-mountain-adventure, interactive-puzzle, shopping, scenic-rail-transport and immersive-curiosity."
    },
    days,
    ...
    operational_flags: {
      red: [
        "Skyland Stables: confirm the Oct 9 ride, and check the operator's current weight and age limits before assuming everyone can ride.",
        "Monticello From Slavery to Freedom is a timed guided tour on a Saturday; book it before anything else on Day 7.",
        "Eckley Miners' Village closes its museum on Tuesdays. The core stop is deliberately the exterior village walk, not the museum.",
        "Steamtown's 2026 short train rides run Friday to Sunday, so the Monday visit cannot be treated as a train ride."
      ],
      yellow: [
        "Wet quartzite cancels Bearfence and the Hickory Run Boulder Field; neither should be forced.",
        "Virginius Island floods. Check trail status rather than assuming access.",
        "The Blue Ridge Tunnel is unlit for 4,273 feet and the floor is uneven; headlamps are required, not optional.",
        "Lackawanna's mine descent is a confined underground space with a fixed return time.",
        "The Lehigh Gorge open-air car is exposed; foliage-season seating sells ahead."
      ],
      green: [
        "Rock House and Holy Land USA as a stone-and-strangeness arrival day.",
        "The Milford comfort break splitting the measured 173-minute Danbury-to-Scranton run.",
        "Eckley into the Boulder Field as one coal-and-geology day.",
        "Harpers Ferry as a single walkable Lower Town, Storer and Virginius system.",
        "Four different ways through the same mountains on Day 6: cavern, car, saddle and rock.",
        "Leesburg replacing Frederick, which fixes the Charlottesville-to-north driving problem and places the shopping block.",
        "Stroudsburg replacing Bethlehem, which keeps Day 10's Hudson run inside the rule.",
        "The Walkway Over the Hudson doubling as a real experience and the Day 10 drive breaker."
      ]
    },
    validation: {
      ...
      hard_closures_or_conflicts: [
        "Day 9 measures 317 baseline driving minutes against a 270-minute standard cap and carries an explicit authorised exception; no single leg on that day exceeds 120 minutes.",
        "The direct Danbury-to-Scranton run measured 173 minutes, over the 150-minute uninterrupted rule, so a documented Milford, PA comfort break splits it.",
        "The direct Charlottesville-to-Leesburg leg measured 153 minutes, over the brief's own 2h30 ceiling, so a documented Warrenton comfort break splits it into 96 and 58 minutes.",
        "Eckley Miners' Village is museum-closed on Tuesday Oct 6; only the exterior village is scheduled.",
        "Four new-core coordinates are town or park centroids rather than exact sites: the Boulder Field, Strasburg Rail Road, Unlocked History and the Dinosaur Footprints.",
        "All forty-two new-core images came from web search with unknown rights and are held private-prototype-only.",
        "OSRM contains no live traffic; the Boston departure and the I-84 and I-80 corridors remain live-traffic gates."
      ],
  ...
  const decisions = {
    ...
    purpose: "Rebuild Route 05 around going under the mountain and inside the machine: coal, caverns, ruins, tunnels, ridges and working transport, with every non-core original preserved and dated.",
    route_dna: "UNDERGROUND × INDUSTRY × RIDGE × RUIN × MACHINE",
    identity_change: "The route's identity becomes UNDERGROUND to INDUSTRY to RIDGE to RUIN to MACHINE. Cultural learning is kept, but it now happens in mines, ruins, tunnels, sacred sites and real terrain rather than in galleries.",
    ...
    source_completeness_note: "The brief status-tags all 31 scheduled Route 05 originals. The package also holds 11 unscheduled V1 alternates the brief does not discuss; they are preserved as documented alternatives with dates rather than being given invented statuses.",
    overnight_changes: "Frederick becomes Leesburg on Oct 11, which turns a 2h45 Charlottesville run into roughly 2h30 and puts the route's one major shopping block exactly where it solves the geometry. Bethlehem becomes Stroudsburg on Oct 12 so the Lehigh Gorge foliage train can anchor the day without stranding the next morning's Hudson run.",
    ...
    drive_correction: "Measured OSRM geometry replaces the brief's prose bands. A documented Milford, PA comfort break splits the Danbury-to-Scranton run, which measured 173 minutes direct. Day 9's 317-minute total carries an explicit authorised cap exception rather than being hidden, and a Warrenton comfort break splits the Charlottesville-to-Leesburg run, which measured 153 minutes direct against the brief's 2h05-2h15 band.",
  ...
  const readme = `# Route 05 — Under the Mountain, Inside the Machine
...
- ${images.length} local carousel images, of which ${rightsGated.length} are rights-gated
- ${baselineMiles.toFixed(1)} OSRM baseline miles
- ${longestLeg} minutes for the longest uninterrupted baseline driving leg
- $${lowCost.toFixed(2)}-$${highCost.toFixed(2)} optional do-everything attraction range per person, before horseback sales tax
- ${museumLikeCore.length} traditional passive museums in the core (${route.experience_model.museum_like_core_percent}%), 0 core food or nightlife
...
Rebuild with \`node scripts/route-05/build.mjs\`, then validate with \`node scripts/validate-route-package.mjs ${ROUTE_SLUG}\`.
`;

  const manifestRoute = manifest.routes.find((entry) => entry.id === ROUTE_ID);
  if (!manifestRoute) throw new Error("route-05 is missing from dataset/manifest.json");
  ...
  await Promise.all([
    ...
    writeJson(path.join(routeDir, "images.json"), { schema_version: "1.0.0", route_id: ROUTE_ID, image_policy: "Legacy rights retained for the original stops. All new-place imagery came from the DuckDuckGo harvester with unknown web rights, so it is held private-prototype-only until permission is cleared.", images }),
    ...
    writeJson(path.join(routeDir, "place-seed.json"), { schema_version: "1.0.0", route_id: ROUTE_ID, generated_from: "Energy Rebuild Routes/Route05_Under_The_Mountain_Inside_The_Machine_Rebuild.md", places }),
    writeFile(path.join(routeDir, "README.md"), readme),
    writeJson(path.join(root, "dataset", "manifest.json"), manifest)
  ]);
  console.log(`Built Route 05 Energy V2: ${places.length} places, ${corePlaces.length} core, ${selectable.length} selectable, ${images.length} images, ${days.length} days, ${baselineMiles} miles.`);
