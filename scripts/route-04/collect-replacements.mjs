...
const { lodgingNodes, manualCoordinates, manualLegs, places, replacementVariants, ROUTE_ID, ROUTE_SLUG, premiumCapMinutes = 210 } = await import(configUrl);
...
async function main() {
  ...
  for (const variant of replacementVariants) {
    ...
    if (variant.unchanged_drive) {
      output.push({ ...variant, node_sequence: [], baseline_total_miles: original.baseline_total_miles, baseline_total_minutes: original.baseline_total_minutes, delta_miles: 0, delta_minutes: 0, cap_minutes: premiumCapMinutes, cap_status: original.baseline_total_minutes <= premiumCapMinutes ? "passes-baseline-cap" : "fails-baseline-cap", routing_note: "Parked-city replacement; bed-to-bed driving is unchanged.", legs: [], geometry: null });
      continue;
    }
    ...
    output.push({
      ...
      cap_minutes: premiumCapMinutes,
      cap_status: minutes <= premiumCapMinutes ? "passes-baseline-cap" : "fails-baseline-cap",
