...
const { dayRoutes, lodgingNodes, manualCoordinates, manualLegs, places, ROUTE_SLUG } = await import(configUrl);
...
async function main() {
  ...
  for (const day of dayRoutes) {
    const result = { day: day.day, risk: day.risk, legs: [], baseline_total_meters: 0, baseline_total_seconds: 0, non_driving_travel_minutes: 0 };
    for (let index = 0; index < day.node_ids.length - 1; index += 1) {
      ...
      const manual = manualLegs[`${fromId}:${toId}`];
      if (manual) {
        const distanceMeters = Math.round(manual.distance_miles * 1609.344);
        result.legs.push({
          ...
          distance_miles: manual.distance_miles,
          baseline_seconds: 0,
          baseline_minutes: 0,
          display_duration_minutes: manual.display_minutes,
          counts_toward_drive_cap: false,
          planning_minutes: { low: manual.display_minutes, high: manual.display_minutes + 45 },
          traffic_risk: "schedule-and-weather",
          geometry: { type: "LineString", coordinates: [from.coordinates, to.coordinates] },
          routing_source: "operator-published crossing time"
        });
        result.non_driving_travel_minutes += manual.display_minutes;
        console.log(`Day ${day.day}: ${fromId} -> ${toId}: ferry ${manual.distance_miles} mi / ${manual.display_minutes} min`);
        continue;
      }
    ...
    result.total_travel_minutes_excluding_queues = result.baseline_total_minutes + result.non_driving_travel_minutes;
